...
void B4aSteppingAction::PrintStep(const G4Step* step) {
  ...
  auto touchable = step->GetPreStepPoint()->GetTouchable();
//  auto depth = touchable->GetHistory()->GetDepth(); //rp - uncomment to get where the particle entered

  auto motherPhysical = touchable->GetVolume(1); // mother
//  auto motherCopyNo = motherPhysical->GetCopyNo(); //rp -  uncomment to cout mother copy number
  auto motherName = motherPhysical->GetName();

  auto thisPhysical = touchable->GetVolume(); // mother
//  auto thisCopyNo = thisPhysical->GetCopyNo(); //rp - uncomment to get cout mother copy
  auto thisName = thisPhysical->GetName();
...
//  auto just_enterd=preStepPoint->GetStepStatus(); //rp - uncomment to get where the particle just entered

  // energy deposit
//  auto edep = step->GetTotalEnergyDeposit(); //rp - uncomment for energy deposit in the step

  G4StepPoint* point1 = step->GetPreStepPoint();
  G4ThreeVector pos1 = point1->GetPosition();
//  double xx=pos1.x(); //rp - uncomment to get x,y,z position of particle at a given step
//  double yy=pos1.y();
//  double zz=pos1.z();

  G4Track* track = step ->GetTrack();
//  G4int steps = track ->GetCurrentStepNumber(); //rp - uncomment for current step of particle

  const G4DynamicParticle* dynamicParticle= track ->GetDynamicParticle();
  G4ParticleDefinition* particle = dynamicParticle->GetDefinition();
  G4String particleName= particle ->GetParticleName();
//  G4double kinEnergy=dynamicParticle->GetKineticEnergy(); //rp - uncomment to get KE of particle
//rp - uncomment the cout below to print the desired information
/*
